...
def get_topic_title(htmlVlue):
    result = re.findall(
        ".*<h1 class=\"zm-editable-content\"(\ ?.*)?>(.*)</h1>.*", htmlVlue)
    if(result):
        return result[0][1]
# 获取当前话题ID
...
def get_parents_title(parents_str):
    ...
    if(d):
        for i in range(0, len(d('a'))):
            _list.append(d('a').eq(i).html().encode(
                'latin1').decode('utf8').strip())
        return _list
# 获取父话题ID
...
def check_subtopic_data(myjson, fatherid):
    ...
    if(isinstance(myjson, dict)):
        ...
        while(len(myjson['msg'][1]) == 11):
            ...
            url = 'https://www.zhihu.com/topic/'+fatherid+'/organize/entire?child=' + \
                myjson['msg'][1][10][0][2]+'&parent='+fatherid
            ...
            myjson = gst.get_subtopic_json(
                url, config.subtopic_values, config.subtopic_headers)
        # 子话题获取完毕后
        wd.wd_temporaryData(myjson)
...
def getonetopic(url, topicid):
    initurl = str('https://www.zhihu.com/topic/'+str(topicid) +
                  '/organize/entire?child='+str(topicid)+'&parent='+str(topicid))
    # 获取页面
...
def starttopictreespider():
    ...
    # 获取子话题数据
    subtopic_json = gst.get_subtopic_json(
        initurl, config.subtopic_values, config.subtopic_headers)
    # 检查子话题数据
    ...
    while(check_topiclist()):
        ...
        print '待 ' + str(len(grabList))
        initurl = baseUrl + '/topic/' + \
            grabList[0]+'/organize/entire#anchor-children-topic'
        initID = grabList[0]
        del grabList[0]
